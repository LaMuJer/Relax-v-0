...
import testFragment from './shaders/fragment.glsl'
...
// // gui.add(material.uniforms.uSize, 'value' , 2, 10, 0.01).name('size').onFinishChange(generateGalaxy)
...
window.addEventListener('scroll' , () => {
    scrollY = (window.scrollY / sizes.height) + 0.1
    gsap.to('#ani', {
        duration: 1,
        opacity: 0,
        y: -100,
    ...
    gsap.to(camera.position, {
        duration: 5,
        ease: 'power2.out',
        x: Math.sin(scrollY * 1.5),
        y: Math.cos(scrollY * 4),
        z: scrollY * 1.5,
    })
})
...
let myAudio = new Audio(audioSrc.src)
